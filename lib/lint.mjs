 ...
 *   words       more than WORDS words on screen; quotes, notes and a figure's
 *               title line are not counted
...
export function lint(entry) {
  ...
  md.slides.forEach((slide, i) => {
    ...
    const counted = layout === 'figure'
      ...
      : slide.content
    const words = onScreen(counted).split(/\s+/).filter(w => /[\p{L}\p{N}]/u.test(w)).length
    if (words > WORDS) warn('words', `${words} words on screen (limit ${WORDS})`)
